...
def initialize_models():
 ...
 if not(os.path.exists('ModelsInput/iEC1364_W_p_coumarate.xml')) or not(os.path.exists('ModelsInput/iJN1463_naringenin_mod.xml')):
  ...
  model=cobra.io.read_sbml_model('iJN1463_naringenin_mod.xml') 
  ...
  # FRU reactions
  model.reactions.get_by_id("EX_fru(e)").bounds=(-15,0)  # Maximize uptake, maximum upper bound  //  DAVID: (-8, 0)
  ...
  model.reactions.get_by_id("4CMCOAS").bounds = (0, 0)
...
def EcoliPputidaFLYCOP_oneConf(sucr1, Ecbiomass, frc2, KTbiomass, fitFunc='MaxNaringenin', maxCycles = 240, dirPlot='', repeat=5):  
  ...
  INPUTS: sucr1: lower bound of sucrose uptake in model 1 (E.coli) (mM)
          ...
          fitFunc: fitness function to optimize. 'MaxNaringenin', maximize Naringenin production by P. putida KT2440
...
# -----------------------------------------------------------------------------
  ...
  for i in range(repeat):
        ...
        # print(title)
        subprocess.run(['../../Scripts/plot_biomassX2_vs_4mediaItem_modGS.sh template2 sucr T4hcinnm fru nar '+str(maxCycles)+' '+title+' blue black darkmagenta yellow EcoliWT KT2440'], shell=True)

        
        # Compute fitness (measure to optimize):
        # (A) DETERMINE ENDCYCLE: when sucrose is exhausted {}
        with open("biomass_vs_sucr_T4hcinnm_fru_nar_template2.txt", "r") as sources: # Creado en ejecución COMETS 'subprocess.run()'
        ...
        # (B) FINAL BIOMASS // FINAL CONCENTRATIONS: pCA, Nar
        ...
        print("Final Ec biomass: ", final_Ecbiomass)
        print("Final KT biomass: ", final_KTbiomass)
        ...
        fitpCA=float(totpCA/final_Ecbiomass)  # Normalized with respect to the final E. coli biomass (mmol / g biomasa)
        fitNar=float(totNar/final_KTbiomass)  # Normalized with respect to the final P. putida KT biomass (mmol / g biomasa)
        ...
        file='IndividualRunsResults/'+'biomass_vs_sucr_T4hcinnm_fru_nar_run'+str(i+1)+'_'+str(fitness)+'_'+str(endCycle)+'.pdf'
        shutil.move('biomass_vs_sucr_T4hcinnm_fru_nar_template2_plot.pdf',file)        
        if(dirPlot != ''):
            file2=dirPlot+'biomass_vs_sucr_T4hcinnm_fru_nar'+str(sucr1)+'_'+str(Ecbiomass)+'_'+str(frc2)+'_'+str(KTbiomass)+'_run'+str(i+1)+'_'+str(fitness)+'_'+str(endCycle)+'.pdf'
  ...
  print("Fitness_function\tconfiguration\t\tfitness\t\tsd\t\tpCA(mM)\t\tNar(mM)\t\tendCycle")  # UNITS: mM (metabolites)  //  gL-1 (biomass)
  print(fitFunc+"\t\t"+str(sucr1)+','+str(Ecbiomass)+','+str(frc2)+','+str(KTbiomass)+"\t"+str(round(avgfitness,6))+"\t"+str(round(sdfitness, 6))+"\t"+str(round(avgpCA,6))+"\t\t"+str(round(avgNar,6))+"\t\t"+str(endCycle))
  with open(dirPlot+"configurationsResults"+fitFunc+".txt", "a") as myfile:
      myfile.write("Fitness_function\tconfiguration\t\tfitness\t\tsd\t\tpCA(mM)\t\tNar(mM)\tendCycle\n")
      myfile.write(fitFunc+"\t\t"+str(sucr1)+','+str(Ecbiomass)+','+str(frc2)+','+str(KTbiomass)+"\t"+str(round(avgfitness,6))+"\t\t"+str(round(sdfitness, 6))+"\t"+str(round(avgpCA,6))+"\t\t"+str(round(avgNar,6))+"\t"+str(endCycle))
  
  print("Avg.fitness(sd): "+str(avgfitness)+"  (+/-"+str(sdfitness)+")")
  if(sdfitness>0.9):  # Correction if SD is too high. Average values for fitness (MaxNaringenin) ~ (¿?). Maximum allowed SD ~ 10% (fitness) = 0.9
      avgfitness=0.0

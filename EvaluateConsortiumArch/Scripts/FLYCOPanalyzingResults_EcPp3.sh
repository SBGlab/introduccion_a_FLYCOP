...
sd_cutoff=`tail log-run${seed}.txt | egrep "p1_sucr1" | awk -F'p1_sucr1' '{print $2}' | cut -d' ' -f18 | sed "s/'//g"`
...
Rscript --vanilla ../Scripts/dataAnalysisPlotsAndTables_EcPp3.r $domainName $id $fitness $sd_cutoff
...
python3 -W ignore ../../Scripts/EcPp3_individualTestFLYCOP_v0.py $param1 $param2 $param3 $param4 $param5 $param6 $param7 $param8 $sd_cutoff $fitness

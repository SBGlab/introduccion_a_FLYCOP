...
def EcoliPputidaFLYCOP_selectConsortiumArchitecture(sucr1, Ecbiomass, Ecbiomass_glyc, frc2, KTbiomass, nh4_Ec, nh4_KT, consortium_arch, \
                                                    fitObj='MaxNaringenin', maxCycles = 240, dirPlot='', repeat=5, cutoff_SD = 0.1):  # At the moment, fitObj and maxCycles have no real utility
  '''
  Call: avgFitness, sdFitness = EcoliPputidaFLYCOP_selectConsortiumArchitecture(sucr1, Ecbiomass, Ecbiomass_glyc, frc2, KTbiomass, nh4_Ec, nh4_KT, consortium_arch)
  ...
  if consortium_arch == "2models" and not ((os.path.exists('ModelsInput/iJN1463_naringeninB12_tmp.mat')) and not (os.path.exists('ModelsInput/iEC1364_W_unique_nar7glu_tmp.mat'))):  # Common strain and exclusive strain of '2models'
      initialize_models_iEC1364_W_unique_nar7glu()
      initialize_models_iJN1463_narB12()
          
  elif consortium_arch == "3models" and not ((os.path.exists('ModelsInput/iJN1463_naringeninB12_tmp.mat')) and not (os.path.exists('ModelsInput/iEC1364_W_exc_glycosilator_tmp.mat')) and not (os.path.exists('ModelsInput/iEC1364_W_p_coumarate_tmp.mat'))):  # Common strain and exclusive strain of '3models'
  ...
  # ================================================
  if consortium_arch == "2models" and not ((os.path.exists('iJN1463_naringeninB12_tmp.mat.txt')) and not (os.path.exists('iEC1364_W_unique_nar7glu_tmp.mat.txt'))):
  ...
  # ================================================
  if consortium_arch == "3models" and not ((os.path.exists('iJN1463_naringeninB12_tmp.mat.txt')) and not (os.path.exists('iEC1364_W_exc_glycosilator_tmp.mat.txt')) and not (os.path.exists('iEC1364_W_p_coumarate_tmp.mat.txt'))):    
...
# ===========================================================================
      ...
      # -----------------------------------------------------------------------
        ...
        # RUNNING COMETS + [R call] Run script to generate one graph:subprocess.call
            if consortium_arch == "2models":
                ...
                baseConfig=str(consortium_arch)+"_"+str(sucr1)+'_'+str(Ecbiomass)+'_'+str(frc2)+'_'+str(KTbiomass)+'_'+str(nh4_Ec)+'_'+str(nh4_KT)
                strains = "EcoliWT KT2440"
                ...
                baseConfig=str(consortium_arch)+"_"+str(sucr1)+'_'+str(Ecbiomass)+'_'+str(Ecbiomass_glyc)+'_'+str(frc2)+'_'+str(KTbiomass)+'_'+str(nh4_Ec)+'_'+str(nh4_KT)
                strains = "EcoliWT EcoliWT_glyc KT2440"
        
        subprocess.run(['../../Scripts/plot_biomassX2_vs_4mediaItem_glic.sh template2 sucr nar7glu fru nar nh4 pi o2 '+str(maxCycles)+' '+baseConfig+' blue black darkmagenta yellow orange aquamarine '+strains], shell=True)
        ...
        # (C) COMPUTE FITNESS: maximize glycosilated naringenin
  ...
  # -------------------------------------------------------------------
  if sdfitness > float(cutoff_SD)*(avgfitness): 
       ID_SD = 1 
  else: ID_SD = 0
  # -------------------------------------------------------------------
  ...
  # ---------------------------------------------------------------------------
  # SAVE RESULTS in TABLE: 'configurationsResults(...).xlsx' file
  # ---------------------------------------------------------------------------
  if not os.path.isfile(dirPlot+"configurationsResults_"+fitObj+".txt"):  # CREATE FILE
  
      myfile = open(dirPlot+"configurationsResults_"+fitObj+".txt", "a")
      # HEADER
      myfile.write("FitObjective\tBaseConfig\tsucr_upt\tInitEc\tInitEc_glyc\tfrc_upt\tInit_KT\tnh4_Ec\tnh4_KT\tConsortium_Arch\t")
      myfile.write("fitFunc\tSD\tID_SD\tGlycNar_mM\tNar_mM\tFinalEc_gL\tFinalEc_glyc_gL\tFinalKT_gL\tInit_Biomass\tFinal_Biomass\t")
      myfile.write("endCycle\tNH4_mM\tpi_mM\tDeadTracking\tDT_cycles\tFinalSucr\tFinalO2\n")  
      
      # INFORMATION LINE
      myfile.write(fitObj+"\t"+baseConfig+"\t"+str(sucr1)+"\t"+str(Ecbiomass)+"\t"+str(Ecbiomass_glyc)+"\t"+str(frc2)+"\t"+str(KTbiomass)+"\t"+str(nh4_Ec)+"\t"+str(nh4_KT)+"\t"+str(consortium_arch)+"\t"+
                   str(round(avgfitness, 6))+"\t"+str(round(sdfitness, 6))+"\t"+str(ID_SD)+"\t"+str(round(avgglycNar, 6))+"\t"+str(round(avgNar, 6))+"\t"+
                   str(round(final_Ecbiomass, 4))+"\t"+str(round(final_Ecbiomass_glyc, 4))+"\t"+str(round(final_KTbiomass, 4))+"\t"+str(round(iniBiomass, 4))+"\t"+str(round(final_biomass, 4))+"\t"+
                   str(endCycle)+"\t"+str(round(Final_nh4, 4))+"\t"+str(round(Final_pi, 4))+"\t"+str(biomass_track)+"\t"+
                   str(dead_process)+"\t"+str(round(sucrConc, 4))+"\t"+str(Final_O2)+"\n")
      myfile.close()
...
# Pendiente revisar, da problemas el ploteo de model.summary() en formato archivo (.write)

def final_model_summary(consortium_arch):
  with open("optimal_model_summary.txt", "w") as model_sum:
      model_sum.write(consortium_arch)
      model_sum.write("\n=======================================\n\n")

...
# CONDITIONAL: number of total strains in the consortium, number of total args received
strain1=${17}
strain2=${18}
if [ $# -eq 19 ]
then
...
# CONDITIONAL: select the number of total strains in the consortium
if [ $# -eq 18 ]
then
	Rscript --vanilla ${dirScripts}/plot.biomassX2.vs.4substrate_glic.r $outFile $plotFile $met1 $met2 $met3 $met4 $met5 $met6 $endCycle $title $colorSubs1 $colorSubs2 $colorSubs3 $colorSubs4 $colorSubs5 $colorSubs6 $strain1 $strain2
elif [ $# -eq 19 ]
then
	Rscript --vanilla ${dirScripts}/plot.biomassX2.vs.4substrate_glic.r $outFile $plotFile $met1 $met2 $met3 $met4 $met5 $met6 $endCycle $title $colorSubs1 $colorSubs2 $colorSubs3 $colorSubs4 $colorSubs5 $colorSubs6 $strain1 $strain2 $strain3
fi 

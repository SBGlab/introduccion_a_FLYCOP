...
def initialize_models_iEC1364_W_unique_nar7glu():
 ...
 if not(os.path.exists('ModelsInput/iEC1364_W_unique_nar7glu.xml')):
  ...
  model.repair()
  cobra.io.save_matlab_model(model,"iEC1364_W_unique_nar7glumod.mat")
  del(model)
  model = cobra.io.load_matlab_model("iEC1364_W_unique_nar7glumod.mat")
  ...
  # SAVE MODEL (tmp)
  cobra.io.save_matlab_model(model,"iEC1364_W_unique_nar7glumod_tmp.mat")
  del(model)
  os.chdir(path)
...
def EcoliPputidaFLYCOP_oneConf(sucr1, Ecbiomass, frc2, KTbiomass, nh4_Ec, nh4_KT, fitFunc='MaxNaringenin', maxCycles = 240, dirPlot='', repeat=5):  
  '''
  Call: avgFitness, sdFitness = EcoliPputidaFLYCOP_oneConf(sucr1, Ecbiomass, frc2, KTbiomass, nh4_Ec, nh4_KT)
  ...
  if not(os.path.exists('ModelsInput/iEC1364_W_unique_nar7glumod_tmp.mat')): # or os.path.exists('ModelsInput/iJN1463_naringeninB12_tmp.mat')): 
  ...
  # ===================================  
  if not(os.path.exists('iEC1364_W_unique_nar7glumod_tmp.mat.txt')) or not (os.path.exists('iJN1463_naringeninB12_tmp.mat.txt')):
    
    # ========================================================================= 
    # MODEL ADAPTATION TO THE PARAMETERS PASSED TO THE 'EcoliPputidaFLYCOP_oneConf' function
    # E. coli
    
    model=cobra.io.load_matlab_model('ModelsInput/iEC1364_W_unique_nar7glumod_tmp.mat')
    ...
    # =========================================================================
    # MAT to COMETS
    mat_to_comets('iEC1364_W_unique_nar7glumod_tmp.mat')
    ...
    massedit.edit_files(['EcPp3_layout_template2.txt'],["re.sub(r'YYY','"+str(KTbiomass)+"',line)"], dry_run=False)
  ...
  sum_glycNar=0  # Glycosilated naringenin quantity variable (production by E.coli)
  ...
  for i in range(repeat):
        ...
        title=str(sucr1)+'_'+str(Ecbiomass)+'_'+str(frc2)+'_'+str(KTbiomass)+'_'+str(nh4_Ec)+'_'+str(nh4_KT)
        subprocess.run(['../../Scripts/plot_biomassX2_vs_4mediaItem_glic.sh template2 sucr nar7glu fru nar nh4 pi o2 '+str(maxCycles)+' '+title+' blue black darkmagenta yellow orange aquamarine EcoliWT KT2440'], shell=True)
        ...
        tot_glicNar=float(finalLineV[4])  # Final glycosilated naringenin (by E.coli)
        tot_Nar=float(finalLineV[6])  # Final Naringenin (by P.putida)
        Final_nh4=float(finalLineV[7])  # First limiting nutrient
        Final_pi=float(finalLineV[8])  # Second limiting nutrient
        Final_O2=float(finalLineV[9])  # Final O2
        ...
        fitNar = tot_glicNar / (final_Ecbiomass + final_KTbiomass) # Final glycosilated naringenin yield over global biomass (both microbes)
        ...
        sum_glycNar += tot_glicNar  # Total glycosiltated naringenin for 'n' repeats
        ...
        if(dirPlot != ''):
            file2=dirPlot+'biomass_vs_sucr_nar7glu_fru_nar_nh4_pi_o2_'+str(sucr1)+'_'+str(Ecbiomass)+'_'+str(frc2)+'_'+str(KTbiomass)+'_run'+str(i+1)+'_'+str(fitness)+'_'+str(endCycle)+'.pdf'
  ...
  print("Fitness_function\tBaseConfig\tfitFunc\tsd\tGlycNar(mM)\tFinalEc(gL-1)\tNar(mM)\tFinalKT(gL-1)\n")  # UNITS: mM (metabolites)  //  gL-1 (biomass)
  
  print(fitFunc+"\t"+str(sucr1)+','+str(Ecbiomass)+','+str(frc2)+','+str(KTbiomass)+","+str(nh4_Ec)+','+str(nh4_KT)+
        "\t"+str(round(avgfitness,6))+"\t"+str(round(sdfitness, 6))+
        "\t"+str(round(avgglycNar,6))+"\t"+str(round(final_Ecbiomass, 4))+
        "\t"+str(round(avgNar,6))+"\t"+str(round(final_KTbiomass, 4))+"\n")
  ...
  if not os.path.isfile(dirPlot+"configurationsResults"+fitFunc+".txt"): 
      ...
      myfile.write("Fitness_function\tBaseConfig\tfitFunc\tsd\tGlycNar_mM\tFinalEc_gL\tNar_mM\tFinalKT_gL\tendCycle\tNH4_mM\tpi_mM\tDeadTracking\tDT_cycles\tFinalSucr\tFinalO2\n")
      
      myfile.write(fitFunc+"\t"+str(sucr1)+','+str(Ecbiomass)+','+str(frc2)+','+str(KTbiomass)+','+str(nh4_Ec)+','+str(nh4_KT)+
                   "\t"+str(round(avgfitness, 6))+"\t"+str(round(sdfitness, 6))+
                   "\t"+str(round(avgglycNar, 6))+"\t"+str(round(final_Ecbiomass, 4))+"\t"+str(round(avgNar,6))+"\t"+str(round(final_KTbiomass, 4))+"\t"+str(endCycle)+
...
# -----------------------------------------------------------------------------
# MODEL SUMMARY
  model = cobra.io.load_matlab_model('iEC1364_W_unique_nar7glumod_tmp.mat')
  model.optimize()

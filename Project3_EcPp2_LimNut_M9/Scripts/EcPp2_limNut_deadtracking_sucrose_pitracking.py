...
# So here it is used a cutoff value of 10.0 mM- clearly disproportionate consumption
...
# -----------------------------------------------------------------------------
  ...
  for i in range(repeat):
        ...
        # DEAD TRACKING
        biomass_track, dead_process = dead_biomass_tracking("biomass_vs_sucr_T4hcinnm_fru_nar_nh4_pi_template2.txt", endCycle = endCycle, n_cycles = 10)
        # Pi OVERCONSUMPTION TRACKING
        pi_overconsumption, pi_cycles = metabolite_tracking_overconsumption("biomass_vs_sucr_T4hcinnm_fru_nar_nh4_pi_template2.txt", 10.0, 9)
  ...
  print("Fitness_function\tconfiguration\t\tfitFunc\t\tsd\t\tpCA(mM)\tFinalEc(gL-1)\tNar(mM)\tFinalKT(gL-1)\tendCycle")  # UNITS: mM (metabolites)  //  gL-1 (biomass)
  ...
  if not os.path.isfile(dirPlot+"configurationsResults"+fitFunc+".txt"): 
      ...
      myfile.write("Fitness_function\tconfiguration\tfitFunc\tsd\tpCA_mM\tFinalEc_gL\tNar_mM\tFinalKT_gL\tendCycle\tNH4_mM\tpi_mM\tDeadTracking\tDT_cycles\tFinalSucr\tPiOverTrackig\tPiOverCycles\n")
      
      myfile.write(fitFunc+"\t"+str(sucr1)+','+str(Ecbiomass)+','+str(frc2)+','+str(KTbiomass)+
                   "\t"+str(round(avgfitness, 6))+"\t"+str(round(sdfitness, 6))+
                   "\t"+str(round(avgpCA, 6))+"\t"+str(round(final_Ecbiomass, 4))+"\t"+str(round(avgNar,6))+"\t"+str(round(final_KTbiomass, 4))+"\t"+str(endCycle)+
                   "\t"+str(round(Final_nh4, 4))+"\t"+str(round(Final_pi, 4))+"\t"+str(biomass_track)+"\t"+dead_process+"\t"+str(sucrConc)+"\t"+str(pi_overconsumption)+"\t"+str(pi_cycles)+"\n")
      myfile.close()
